...
djob.add_condor_cmd('requirements', '(regexp("cn[^1]\d*.sarathi.iucaa.in", TARGET.Machine))')
...
for i in np.arange(20, 40):
    ...
    write_node(i, outputbank, log_files, err_files, out_files, start, end, crawl, '5G')
...
for i in np.arange(80, 120):
    ...
    write_node(i, outputbank, log_files, err_files, out_files, start, end, crawl, '10G')

for i in np.arange(120, 200):
    step = 0.5
    crawl = step
    start = step * i
    end = step * (i + 1)
